...
解决XSS攻击的手段就是将浏览器敏感参数进行编译，编译的API有三套

| 编码               | 解码               | 说明                                                         |
| ------------------ | ------------------ | ------------------------------------------------------------ |
| encodeURI          | decodeURI          | 编码能力较弱，只能把URL地址中的中文或空格等编译，可用于整个URL的编译，【服务端和客户端都可用】 |
| encodeURIComponent | decodeURIComponent | 编码能力较强，可以多编译一些特殊符号，常用于对参数的编译【服务端和客户端都可用】 |
| escape             | unescape           | 只合适客户端之间的通信和编码                                 |
